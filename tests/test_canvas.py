...
def test_digit_feedback(client):
    ...
    # Prediction
    submit_digit_response = submit_prediction(client, 'digit', test_image)

    # Check if the prediction is success or not
    assert "prediction" in submit_digit_response
    assert "confidence" in submit_digit_response
    assert "probabilities" in submit_digit_response

    predicted_label = submit_digit_response['prediction']
    confidence = submit_digit_response['confidence']
